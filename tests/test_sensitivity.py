...
def test_grid_nan_where_wacc_le_growth():
    df = sensitivity_grid(
        fcff_stage1=[100.0],
        rate_range=(0.02, 0.04, 0.02),
        growth_range=(0.03, 0.05, 0.02),
        fade_years=0,
    )
    # discount_rate 0.02 <= growth 0.03/0.05 -> NaN
...
def test_grid_centre_cell_matches_headline_fair_value():
    ...
    centre = run.sensitivity.loc[round(assumptions.discount_rate, 8), round(assumptions.terminal_growth, 8)]
